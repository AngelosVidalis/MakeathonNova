...
from code.db import conn
import json, wikipedia
from code.get_articles import get_links, get_text, save_file
...
def embed_text(text): #split to chunks and insert vectors into data base
...
def run_query(query):
    ...
    You will be equipped with a wide range of news articles or similar tools to help you, which you use to research your answer. 
    You should focus on serving the user's needs as best you can, which will be wide-ranging.You should choose the most important information out of the given request's keywords.
    ...
    print("Final answer:")
    print(response.text) #response of AI

    save = input("Save answer? (y/n): ")  #save answer as pdf/txt/json
    ...
    while True:
        ...
        if option == 1:
            ...
            if ans == 1:
                ...
                category = str(input("Give me a category (technology / finance / politics etc.): "))   #category 
                ...
                for page_var in range(stop + 1):
                    ...
                    links = [current_source + l for l in get_links(current_page)]   # links = get(current_page) \nfor i in range(len(links)): links[i] = DEFAULT_SOURCE + links[i]
